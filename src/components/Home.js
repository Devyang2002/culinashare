...
function Home() {
  ...
  const handleRecipeMarkFav = async({e, recipe_id}) => {
    ...
    try {
      const response = await fetch(`https://culina-share-deha.vercel.app/api/v1/recipes/${recipe_id}/markFavorite/${user_id}`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchRecipes = async () => {
      try {
        const response = await fetch(`https://culina-share-deha.vercel.app/api/v1/recipes/fetchAll`); 
